...
const AboutProgress = () => {
    ...
    return (
        <div className={'space-y-8 py-6 md:py-10'}>
            {data.map((item, index)=> (
                <div data-aos="fade-up" data-aos-delay={450 + (index + 1) * 150} data-aos-anchor-placement="top-center" key={item.id} className={'flex items-center gap-6 bg-white rounded-xl shadow-md p-6'}>
                    ...
                    <div className={'flex-1'}>
                        ...
                        <div className={'w-full h-2 bg-gray-300 rounded-full overflow-hidden'}>
                            <div className={'h-2 bg-[#b58b58] rounded-full'} style={{width:`${item.percent}%`}}></div>
                        </div>
                    </div>
                </div>
            ))}
